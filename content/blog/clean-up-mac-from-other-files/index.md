...
GB. Also, save the second list to another file so that we are only doing it once.
...
And Now, start finding the ones which are big one by one. Let's say one of the entry was 5.6G, Let's find which one was it.

```sh
cat ~/space. log | fgrep -f highspace. log #It will give something like this: 1.2G./.Android/avd/Nexus_6_API_21.avd
2.7G./.Android/avd
2.7G./.Android
```
